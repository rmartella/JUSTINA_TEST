#ifndef SRC_PATH_CALCULATOR_H_
#define SRC_PATH_CALCULATOR_H_
...
void computeVoronoi(DynamicVoronoi *voronoi_, costmap_2d::Costmap2D* costmap_,
	...
	ROS_ERROR("voronoi.initializeMap");
	voronoi_->initializeMap(sizeX, sizeY, map);
	ROS_ERROR("Time (for initializeMap): %f sec",
			(ros::Time::now() - t).toSec());
	t = ros::Time::now();

	ROS_ERROR("voronoi.update");
	voronoi_->update(); // update distance map and Voronoi diagram
	ROS_ERROR("Time (for update): %f sec", (ros::Time::now() - t).toSec());
	t = ros::Time::now();

	ROS_ERROR("voronoi.prune");
	if (doPrune)
		voronoi_->prune();  // prune the Voronoi
	ROS_ERROR("Time (for prune): %f sec", (ros::Time::now() - t).toSec());
}
...
bool makePlan(costmap_2d::Costmap2D* costmap_, DynamicVoronoi * voronoi_,
	...
	bool ** map = computeMapConvert(voronoi_, costmap_);
	...
	if (!voronoi_->isVoronoi(goal_x, goal_y)) {
		res3 = findPath(&path3, goal_x, goal_y, start_x, start_y, voronoi_, 0,
				1);
	...
	if (!voronoi_->isVoronoi(start_x, start_y)) {
		res1 = findPath(&path1, start_x, start_y, goal_x, goal_y, voronoi_, 0,
				1);
...
nav_msgs::Path publishPlan(
		const std::vector<geometry_msgs::PoseStamped>& path_poses,
